...
void * thread_c(void * arg_in)
{
    ...
    while(1){
        ...
        int check_result = gmn_check(&gmn, arg->guess);
        if (check_result == 0){
            strncpy(arg->message, gmn_get_message(&gmn), MSG_BUF_SIZE);
            arg->message[MSG_BUF_SIZE - 1] = '\0';
            arg->result = 0;
            pthread_cond_signal(&arg->cond_guess);
            break;
        } else {
...
int main(int argc, char *argv[])
{
    ...
    check_pthread_return(pthread_create(&threadC, NULL, thread_c, &arg), "Creating thread C");
    check_pthread_return(pthread_create(&threadP, NULL, thread_p, &arg), "Creating thread P");
