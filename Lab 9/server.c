...
void * thread_main(void * arg_in)
{
    ...
    if (send_all(sockfd, buffer, strlen(buffer)) == -1) {
        perror("server: send max value");
        goto cleanup;
    }
        while (1) {

        if (recv_lines(sockfd, buffer, sizeof(buffer)) == -1) {
            perror("server: recv guess");
            goto cleanup;
        }

        if (sscanf(buffer, "%d", &guess) != 1) {
            fprintf(stderr, "server: invalid guess received\n");
            goto cleanup;
        }
        ...
        if (result == 0) {
            snprintf(buffer, sizeof(buffer), "%s", gmn_get_message(&gmn));
            if (send_all(sockfd, buffer, strlen(buffer)) == -1) {
                perror("server: send final message");
            }
            break;
        }
    }
