...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    if (write(fdc[PIPEFD_WRITE], &max, sizeof(int)) == -1)
        die("Child fail write max value");
    int guess, result;
    while (1){
        if (read(fdp[PIPEFD_READ], &guess, sizeof(int)) != sizeof(int))
            die("Child fail read from parent");
        result = gmn_check(&gmn, guess);

        if (write(fdc[PIPEFD_WRITE],&result, sizeof(int)) == -1)
            die("Child fail write from parent");
        if (result == 0){
            break;
        }
    }
    ...
    if (write(fdc[PIPEFD_WRITE], final_message, strlen(final_message)) == -1)
        die("Child fail to write final message to parent");
...
int main(int argc, char *argv[])
{
    ...
    if (read(fdc[PIPEFD_READ], &max, sizeof(int)) != sizeof(int))
        die("Parent fail to read max value from child");
    do { 
        guess = (min + max)/2;
        printf("My guess: %d\n", guess);
        ...
        if (write(fdp[PIPEFD_WRITE], &guess, sizeof(int)) == -1)
            die("Parent fail write child");
        if (read(fdc[PIPEFD_READ], &result, sizeof(int)) != sizeof(int))
            die("Parent fail read child");
        if (result > 0)
            min = guess + 1;
        else if (result < 0)
            max = guess - 1;
    ...
    ssize_t bytes_read = read(fdc[PIPEFD_READ], final_message, MSG_BUF_SIZE - 1);
    if (bytes_read == -1)
        die("Parent fail to read final message");
    final_message[bytes_read] = '\0';
